...
class XinController extends BaseController {
  ...
  async list () {
    ...
    if (!this.xinConfig?.official?.saas_table_table_id) {
      this.ctx.body = this.app.xinError.basicError(null, '数据表配置异常')
      return
    }
    const result = await this.service.official.table.list(bodyData)
  ...
  async detail () {
    ...
    if (!filter._id) {
      this.ctx.body = this.app.xinError.basicError(null, '数据表ID缺失')
      return
    }

    const result = await this.service.table.main.detail({ _id: filter._id, })
