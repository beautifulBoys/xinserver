...
class XinController extends BaseController {
  ...
  async list () {
    ...
    if (!this.xinConfig?.official?.saas_connector_table_id) {
      this.ctx.body = this.app.xinError.basicError(null, '连接器表配置异常')
      return
    }
    const result = await this.service.official.connector.list(bodyData)
