...
class XinController extends BaseController {
  ...
  async list () {
    ...
    if (!this.xinConfig?.official?.saas_customapi_table_id) {
      this.ctx.body = this.app.xinError.basicError(null, '自定义接口表配置异常')
      return
    }
    const result = await this.service.official.customapi.list(bodyData)
