...
class XinController extends BaseController {
  ...
  async list () {
    ...
    if (!this.xinConfig?.official?.saas_plugin_table_id) {
      this.ctx.body = this.app.xinError.basicError(null, '插件表配置异常')
      return
    }
    const result = await this.service.official.plugin.list(bodyData)
  ...
  async detail () {
    ...
    const result = await this.service.table.record.detail({ _id: filter._id, })
    this.ctx.body = this.app.xinError.success(result)
  }
