...
class XinController extends BaseController {
  ...
  async list () {
    ...
    if (!this.xinConfig?.official?.saas_screen_table_id) {
      this.ctx.body = this.app.xinError.basicError(null, '可视化表配置异常')
      return
    }
    const result = await this.service.official.screen.list(bodyData)
