...
class XinService extends BaseService {
  ...
  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    ...
    const listRes = this.app.models.xin_expand_token.Model
                            .find(filterQuery)
                            .select(select)
                            .skip(skip)
                            .limit(limit)
                            .populate([
                              { path: 'authUserInfo', select: '_id name username' },
                            ...
                            .lean()
                            .exec()
    const countRes = await this.app.models.xin_expand_token.Model.countDocuments(filterQuery).exec()
  ...
  async count (filter = {}) {
    ...
    const result = await this.app.models.xin_expand_token.Model.countDocuments(filterQuery).exec()
  ...
  async add (data = {}) {
    ...
    const result = await new this.app.models.xin_expand_token.Model({
      ...(data || {}),
  ...
  async update ({ filter = {}, data = {}, }) {
    ...
    const result = await this.app.models.xin_expand_token.Model
                                  .findOneAndUpdate(
                                    filterQuery,
                                    updateQuery,
                                    { new: true }
