...
class XinService extends BaseService {
  ...
  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    ...
    const listRes = this.app.models.xin_warning.Model
                            .find(filterQuery)
                            .select(select)
                            .skip(skip)
                            .limit(limit)
                            .populate([
                              { path: 'tableInfo', select: '_id name' },
                              { path: 'tableRecordInfo', select: '_id status' },
                            ...
                            .lean()
                            .exec()
    const countRes = await this.app.models.xin_warning.Model.countDocuments(filterQuery).exec()
  ...
  async count (filter = {}) {
    ...
    const result = await this.app.models.xin_warning.Model.countDocuments(filterQuery).exec()

    return result || 0
  }

  async add ({ table_id, table_record_id, type, level, body, description, }) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const warningTypeMap = this.app.xinConfig.warningTypeMap

    const result = await new this.app.models.xin_warning.Model({
      table_id,
      table_record_id,
      name: warningTypeMap[type]?.name,
      type,
      body,
      level,
      description,
  ...
  async update ({ filter = {}, data = {}, }) {
    ...
    const result = await this.app.models.xin_warning.Model
                                  .findOneAndUpdate(
                                    filterQuery,
                                    updateQuery,
                                    { new: true }
