...
class XinService extends BaseService {
  ...
  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    ...
    const listRes = this.app.models.xin_resource_group.Model
                            .find(filterQuery)
                            .select(select)
                            .skip(skip)
                            .limit(limit)
                            ...
                            .lean()
                            .exec()
    const countRes = this.app.models.xin_resource_group.Model.countDocuments(filterQuery).exec()
  ...
  async count (filter = {}) {
    ...
    const result = await this.app.models.xin_resource_group.Model.countDocuments(filterQuery).exec()
  ...
  async add (data = {}) {
    ...
    const result = await new this.app.models.xin_resource_group.Model({
      ...(data || {}),
  ...
  async update ({ filter = {}, data = {}, }) {
    ...
    const updateQuery = this._emptyValueFilters({
      ...(data || {}),
      parent_id: undefined,
      ...(data.parent_id === 'root' ? {$unset: { parent_id: 1 }} : { parent_id: data.parent_id }),
      update_time: Date.now(),
    })


    const result = await this.app.models.xin_resource_group.Model
                                  .findOneAndUpdate(
                                    filterQuery,
                                    updateQuery,
                                    { new: true }
