...
class XinService extends BaseService {
  ...
  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    ...
    const listRes = this.app.models.xin_resource.Model
                            .find(filterQuery)
                            .select(select)
                            .skip(skip)
                            .limit(limit)
                            .populate([
                              { path: 'groupInfo', select: '_id name' },
                            ...
                            .lean()
                            .exec()
    const countRes = this.app.models.xin_resource.Model.countDocuments(filterQuery).exec()
    const diskSize = this.service.resource.main.allSize({ status: true })

    const result = await Promise.all([ listRes, countRes, diskSize, ])
    return { data: result[0], count: result[1], size: result[2] }
  }
  ...
  async allSize (filter = {}) {
    ...
    const sizeRes = await this.app.models.xin_resource.Model.aggregate([
      { $match: filterQuery },
      { $group: { _id: null, totalSize: { $sum: '$size' } } }
    ])

    return sizeRes?.[0]?.totalSize || 0
  }
