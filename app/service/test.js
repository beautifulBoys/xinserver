...
class XinService extends BaseService {
  ...
  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    ...
    const listRes = this.app.models.xin_test.Model
                            .find(filterQuery)
                            .select(select)
                            .skip(skip)
                            .limit(limit)
                            ...
                            .lean()
                            .exec()
    const countRes = await this.app.models.xin_test.Model.countDocuments(filterQuery).exec()
  ...
  async count (filter = {}) {
    ...
    const result = await this.app.models.xin_test.Model.countDocuments(filterQuery).exec()
  ...
  async add (data = {}) {
    ...
    const result = await new this.app.models.xin_test.Model({
      ...(data || {}),
