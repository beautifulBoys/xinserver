...
class XinService extends BaseService {
  ...
  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    ...
    const listRes = this.app.models.xin_organization.Model
                            .find(filterQuery)
                            .select(select)
                            .skip(skip)
                            .limit(limit)
                            .populate([
                              ...
                              { path: 'parentInfo', select: '_id name' },
                              { path: 'managerUserInfo', select: '_id name avatar nickname' },
                            ])
                            .sort({
                              ...(sort || {}),
                            })
                            .lean()
                            .exec()
    const countRes = await this.app.models.xin_organization.Model.countDocuments(filterQuery).exec()
  ...
  async count (filter = {}) {
    ...
    const result = await this.app.models.xin_organization.Model.countDocuments(filterQuery).exec()
  ...
  async add (data = {}) {
    ...
    const result = await new this.app.models.xin_organization.Model({
      ...(data || {}),
  ...
  async update ({ filter = {}, data = {}, }) {
    ...
    const result = await this.app.models.xin_organization.Model
                                  .findOneAndUpdate(
                                    filterQuery,
                                    updateQuery,
                                    { new: true }
