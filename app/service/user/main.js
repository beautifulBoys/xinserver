'use strict';

const BaseService = require('../../core/baseService')

class XinService extends BaseService {

  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined
    

    const limit = pageSize || 20
    const skip = limit * ((current || 1) - 1)

    const filterQuery = this._mongoFilterCreate({
      status: filter.status === false ? false : true,
      usertype: filter.usertype || 'outside',
      ...(filter || {}),
    })

    // console.log('--', filterQuery)

    const listRes = this.app.models.xin_user.Model
                            .find(filterQuery)
                            .select(
                              Object.assign({
                                wx_openid: 0,
                                password: 0,
                                token: 0,
                              }, select)
                            )
                            .skip(skip)
                            .limit(limit)
                            .populate([
                              { path: 'organizationInfos', select: '_id name', },
                              { path: 'roleInfos', select: '_id name value', },
                              { path: 'tagInfos', select: '_id name', },
                              { path: 'createUserInfo', select: '_id nickname name' },
                            ])
                            .sort({
                              ...(sort || {}),
                            })
                            .lean()
                            .exec()
    const countRes = await this.app.models.xin_user.Model.countDocuments(filterQuery).exec()

    const result = await Promise.all([ listRes, countRes ])
    return { data: result[0], count: result[1], }
  }

  async exist (data) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      status: filter.status === false ? false : true,
      ...(data || {}),
    })

    const result = this.app.models.xin_user.Model
                            .findOne(filterQuery)
                            .select(
                              Object.assign({
                                wx_openid: 0,
                                password: 0,
                                token: 0,
                              })
                            )
                            .exec()
    return result
  }

  async count (filter = {}) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      usertype: filter.usertype || 'outside',
      ...(filter || {}),
    })

    const result = await this.app.models.xin_user.Model.countDocuments(filterQuery).exec()

    return result || 0
  }

  async add (data = {}) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const result = await new this.app.models.xin_user.Model({
      ...(data || {}),
      job_no: data.job_no || await this.service.system.project.getJobNo(),
      create_user_id: token_user_id,
    }).save()

    return result?.toObject?.()
  }

  async mine () {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      _id: token_user_id,
      status: true,
    })

    const result = await this.app.models.xin_user.Model
                                  .findOne(filterQuery)
                                  .select(
                                    Object.assign({
                                      // wx_openid: 0,
                                      password: 0,
                                    })
                                  )
                                  .populate([
                                    { path: 'organizationInfos', select: '_id name', },
                                    { path: 'roleInfos', select: '_id name value', },
                                    { path: 'tagInfos', select: '_id name', },
                                  ])
                                  .lean()
                                  .exec()
    return result
  }

  async update ({ filter = {}, data = {}, }) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      ...(filter || {}),
    })

    const updateQuery = this._emptyValueFilters({
      ...(data || {}),
      update_time: Date.now(),
    })


    const result = await this.app.models.xin_user.Model
                                  .findOneAndUpdate(
                                    filterQuery,
                                    updateQuery,
                                    { new: true }
                                  )
                                  .select({ _id: 1, })
                                  .lean()
                                  .exec()

    return result
  }

  async batchUpdate ({ filter = {}, data = {}, }) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      ...(filter || {}),
    })

    const updateQuery = this._emptyValueFilters({
      ...(data || {}),
      update_time: Date.now(),
    })

    const result = await this.app.models.xin_user.Model
                                  .updateMany(
                                    filterQuery,
                                    updateQuery,
                                    { new: true }
                                  )
                                  .select(
                                    Object.assign({
                                      wx_openid: 0,
                                      password: 0,
                                      token: 0,
                                    })
                                  )
                                  .exec()

    return result
  }

  async detail (filter = {}) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      ...(filter || {}),
    })

    const result = await this.app.models.xin_user.Model
                                  .findOne(filterQuery)
                                  .select(
                                    Object.assign({
                                      wx_openid: 0,
                                      password: 0,
                                      token: 0,
                                    })
                                  )
                                  .populate([
                                    { path: 'organizationInfos', select: '_id name', },
                                    { path: 'roleInfos', select: '_id name value', }
                                  ])
                                  .lean()
                                  .exec()

    return result
  }

  async delete (filter = {}) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      ...(filter || {}),
    })

    const result = await this.app.models.xin_user.Model
                                  .findOneAndDelete(filterQuery, {
                                    projection: { _id: 1, },
                                    lean: true,
                                  })
                                  .exec()

    return result
  }

  async batchDelete (filter = {}) {
    const token_user_id = this.ctx.xinToken?.user_id || undefined

    const filterQuery = this._mongoFilterCreate({
      ...(filter || {}),
    })

    const result = await this.app.models.xin_user.Model
                                  .deleteMany(filterQuery)
                                  .exec()

    return result
  }

}

module.exports = XinService
