...
class XinService extends BaseService {
  ...
  async list ({ filter = {}, select = {}, pageSize = 20, current = 1, sort = {}, }) {
    ...
    const listRes = this.app.models.xin_permission.Model
                            .find(filterQuery)
                            .select(select)
                            .skip(skip)
                            .limit(limit)
                            ...
                            .lean()
                            .exec()
    const countRes = await this.app.models.xin_permission.Model.countDocuments(filterQuery).exec()
  ...
  async count (filter = {}) {
    ...
    const result = await this.app.models.xin_permission.Model.countDocuments(filterQuery).exec()
  ...
  async add (data = {}) {
    ...
    const result = await new this.app.models.xin_permission.Model({
      ...(data || {}),
  ...
  async update ({ filter = {}, data = {}, }) {
    ...
    const result = await this.app.models.xin_permission.Model
                                  .findOneAndUpdate(
                                    filterQuery,
                                    updateQuery,
                                    { new: true }
  ...
  async batchUpdate ({ filter = {}, data = {}, }) {
    ...
    const filterQuery = this._mongoFilterCreate({
      ...(filter || {}),
      _ids: undefined,
      _id: filter._ids?.length ? { $in: filter._ids } : undefined,
    })
